# Nestwell × ~/life 동기화 설계

작성일: 2026-09-18 · 상태: 승인됨(브레인스토밍) · 다음 단계: 구현 계획

## 1. 목적

1. Nestwell 앱(v6)에 입력하는 모든 데이터(식단·운동·돈·설정·회고)가 `~/life` 에 기록으로 남는다.
...
```
[폰 / PC 브라우저] ─> Nestwell 웹앱 (공개 저장소 nestwell → GitHub Pages, 코드만)
...
| 저장소 | 공개 | 담는 것 |
|---|---|---|
| `sasaway/nestwell` | 공개 (무료 Pages 조건) | 앱 코드만. 데이터·토큰 없음 |
...
1. 저장소 생성(gh 로 대신 가능 — 실행 전 확인): `nestwell`(공개), `life`(비공개)
2. `nestwell` 의 GitHub Pages 켜기
